...
- The old baselines (`goldens/*.png`, 19 images) were deleted and replaced by 19 portable `goldens/ci/<name>.png` (committed) and 19 readable `goldens/macos/<name>.png` (generated locally, currently git-ignored). Names are now snake case without spaces or dashes.
...
**Where the readable images live.** The repository `.gitignore` already ignores `**/goldens/macos` (rule "Ignore platform-specific goldens", from before this PR), so the readable macOS set is generated locally but is **not committed**; the committed baselines are the portable `goldens/ci` set that CI checks. Whether to commit the readable set is an open owner decision (trade-offs listed in the PR discussion). alchemist names the folder after the host operating system on purpose: readable images differ between operating systems, so they are only checked on macOS (`platforms: {HostPlatform.macOS}`); on Windows and Linux only the portable `goldens/ci` set runs. `GoldenDevice.tabletPortrait` was aligned with golden_toolkit's size (1024x1366) and `tabletLandscape` added. A Claude skill documenting the old golden_toolkit setup was added at `.claude/skills/golden-toolkit-legacy/SKILL.md` (reference commit `211cc25`).
