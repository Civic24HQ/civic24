...
**Verification.** Built app inspected for all three flavors (`flutter build ios --simulator --debug`; production only as a Debug build, nothing signed in): display names as above, URL scheme equals each flavor's plist `REVERSED_CLIENT_ID`, `GIDClientID` absent, `PrivacyInfo.xcprivacy` present (development). **Sign-in (owner, iPhone 17 Pro simulator):** development flavor before the change (baseline on `develop`) and after it, and the **production flavor (Debug build, production Firebase project) after the final commits**: "Continue with Google" opened the sheet, returned to the app and logged in. This also counts as use of the production iOS OAuth client (see the Google Auth Platform notice above). `melos` bootstrap-free checks run separately: generated files up to date, format clean, analyze clean, `melos run flutter:test` passes. `melos run ci:check` as one command hung for 1 h 27 min at 0% CPU and was killed. Its Dart processes were not all killed and stayed idle for over two hours; they were found and stopped before the final run. Cause of the hang not diagnosed (suspect: it ran while Flutter builds were running). After that, bootstrap, generated files, format, analyze and tests were run one at a time and all passed, again after the last commit.

**Not verified:** sign-in on staging (production and development were tested); the URL scheme in Release builds (a Release production build is deliberately not made locally); Sign in with Apple (suspended, no entitlement); running from the Xcode app (see the note below); whether App Check is enforced (sign-in and the feed worked on the simulator); the privacy manifest against Apple's validator (needs App Store Connect); runtime permission prompts.
...
**Review outcome (CodeRabbit, 3 threads, all resolved):** (1) pbxproj script ordering: partly adopted, the registration phase now declares its input files and fails if the write fails; `outputPaths` on the flavor-copy phase not adopted because it has no declared inputs and could leave a stale plist. (2) Location prompt wording: not changed as suggested, because nothing attaches coordinates to issue reports; the string was reworded to say what the profile screen does, and the stale AGENTS.md rule (§6.2) was corrected. (3) Required-reason API declarations: not added; the app's own code uses none, about 40 SDK bundles carry their own manifests, and the plugins without one were scanned. `NSPrivacyAccessedAPITypes` stays as an explicit empty array. Follow-up changes made because of the review: precise location removed from the privacy manifest (the only reader of the location, the profile screen's "use accurate location" button, is commented out); the location string now reads formally.

**Finding: location is not requested anywhere in the UI today.** `getAccurateLocationData` is only called from a commented-out button (`complete_profile_view.dart`, TODO to refactor the UX). The `NSLocationWhenInUseUsageDescription` key stays because the location code and plugins are still in the app and `permission_handler_apple` keys location support off it. When the button ships, add a precise or coarse location entry to `PrivacyInfo.xcprivacy` and keep the string accurate.

**Note: running from the Xcode app.** `permission_handler_apple` finds `Info.plist` from the working directory of the process that resolves packages. `flutter run` and `xcodebuild` start in the project; the Xcode app starts in `/`. Tested with the package manifest run from `/` (through the project's own symlink, manifest cache off): camera, photos and location resolve to 0 and notifications to 1, so those permissions would report denied without any error. The documented fix, checked the same way with the variable set, is `launchctl setenv PERMISSION_HANDLER_INFO_PLIST <abs path to ios/Runner/Info.plist>`, then quit Xcode and `rm -rf ~/Library/Developer/Xcode/DerivedData`. The Xcode window itself was not run.
