...
class CommentSheet extends StackedView<CommentSheetModel> {
  const CommentSheet({required this.completer, required this.request, super.key});

  final SheetRequest<dynamic> request;
  final Function(SheetResponse<dynamic>) completer;

  @override
  Widget builder(BuildContext context, CommentSheetModel viewModel, Widget? child) {
          ...
          children: [
            ...
            AppTextField(
              ...
              suffixIcon: IconButton(
                onPressed: () {},
                icon: Icon(Icons.send, size: AppDimensions.size24, color: context.primary),
              ),
            ),
          ],
        ),
