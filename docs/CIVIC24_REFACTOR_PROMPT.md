...
3. iOS: confirm each flavor's `GoogleService-Info.plist` is in place and copied correctly, that `IOS_CLIENT_ID` in each secrets JSON matches that plist's `CLIENT_ID`, that the built app's URL scheme matches its `REVERSED_CLIENT_ID` (the build phase does this), and that the plist really contains a `REVERSED_CLIENT_ID`; also that the reversed client ID URL scheme is registered in `Info.plist`, and that the OAuth iOS client exists in Google Cloud for each bundle ID.
