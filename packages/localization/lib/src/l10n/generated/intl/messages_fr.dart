// DO NOT EDIT. This is code generated via package:intl/generate_localized.dart
// This is a library that provides messages for a fr locale. All the
// messages from the main program should be duplicated here with the same
// function name.

// Ignore issues from commonly used lints in this file.
// ignore_for_file:unnecessary_brace_in_string_interps, unnecessary_new
// ignore_for_file:prefer_single_quotes,comment_references, directives_ordering
// ignore_for_file:annotate_overrides,prefer_generic_function_type_aliases
// ignore_for_file:unused_import, file_names, avoid_escaping_inner_quotes
// ignore_for_file:unnecessary_string_interpolations, unnecessary_string_escapes

import 'package:intl/intl.dart';
import 'package:intl/message_lookup_by_library.dart';

final messages = new MessageLookup();

typedef String MessageIfAbsent(String messageStr, List<dynamic> args);

class MessageLookup extends MessageLookupByLibrary {
  String get localeName => 'fr';

  static String m0(action) => "Appuyez pour ${action}";

  static String m1(version) => "(v${version})";

  static String m2(emailValue) =>
      "Un e-mail de réinitialisation du mot de passe a été envoyé à ${emailValue}. Veuillez vérifier votre boîte de réception et suivre les instructions pour réinitialiser votre mot de passe.";

  final messages = _notInlinedMessages(_notInlinedMessages);
  static Map<String, Function> _notInlinedMessages(_) => <String, Function>{
    "featureAddReport": MessageLookupByLibrary.simpleMessage(
      "Ajouter un signalement",
    ),
    "featureAddReportCategory": MessageLookupByLibrary.simpleMessage(
      "Sélectionnez la catégorie du rapport",
    ),
    "featureAddReportContent": MessageLookupByLibrary.simpleMessage(
      "Partagez les détails du problème civique",
    ),
    "featureAddReportContentHint": MessageLookupByLibrary.simpleMessage(
      "Partagez vos remarques",
    ),
    "featureAddReportMedia": MessageLookupByLibrary.simpleMessage(
      "Veuillez télécharger une image claire de votre signalement",
    ),
    "featureAlreadyHaveAnAccount": MessageLookupByLibrary.simpleMessage(
      "Vous avez déjà un compte ?",
    ),
    "featureBookmarkedReports": MessageLookupByLibrary.simpleMessage("Favoris"),
    "featureCategoryTypeAbandoned": MessageLookupByLibrary.simpleMessage(
      "Projets abandonnés",
    ),
    "featureCategoryTypeAbandonedDescription":
        MessageLookupByLibrary.simpleMessage(
          "Signalez des projets gouvernementaux ou communautaires inachevés ou abandonnés dans votre localité.",
        ),
    "featureCategoryTypeDetailedAbandoned":
        MessageLookupByLibrary.simpleMessage("Projets abandonnés & inachevés"),
    "featureCategoryTypeDetailedEducation":
        MessageLookupByLibrary.simpleMessage("Éducation & équipements publics"),
    "featureCategoryTypeDetailedGovernance":
        MessageLookupByLibrary.simpleMessage("Gouvernance"),
    "featureCategoryTypeDetailedHealth": MessageLookupByLibrary.simpleMessage(
      "Santé & assainissement",
    ),
    "featureCategoryTypeDetailedLight": MessageLookupByLibrary.simpleMessage(
      "Problèmes d\'éclairage & d\'électricité",
    ),
    "featureCategoryTypeDetailedRoad": MessageLookupByLibrary.simpleMessage(
      "Problèmes de routes & transports",
    ),
    "featureCategoryTypeDetailedSecurity": MessageLookupByLibrary.simpleMessage(
      "Problèmes de sécurité",
    ),
    "featureCategoryTypeDetailedUrban": MessageLookupByLibrary.simpleMessage(
      "Urbanisme & violations de zonage",
    ),
    "featureCategoryTypeDetailedWaste": MessageLookupByLibrary.simpleMessage(
      "Déchets & dangers environnementaux",
    ),
    "featureCategoryTypeEducation": MessageLookupByLibrary.simpleMessage(
      "Éducation",
    ),
    "featureCategoryTypeEducationDescription":
        MessageLookupByLibrary.simpleMessage(
          "Signalez des bâtiments scolaires délabrés, des salles communautaires fermées ou des bibliothèques négligées.",
        ),
    "featureCategoryTypeGovernance": MessageLookupByLibrary.simpleMessage(
      "Gouvernance",
    ),
    "featureCategoryTypeGovernanceDescription":
        MessageLookupByLibrary.simpleMessage(
          "Signalez des problèmes tels que des retards administratifs, une mauvaise prestation de services, des pratiques corrompues ou un mauvais usage des fonds publics.",
        ),
    "featureCategoryTypeHealth": MessageLookupByLibrary.simpleMessage(
      "Santé & assainissement",
    ),
    "featureCategoryTypeHealthDescription":
        MessageLookupByLibrary.simpleMessage(
          "Signalez des toilettes publiques sales, une mauvaise hygiène dans les cliniques, des eaux usées à ciel ouvert ou des conditions sanitaires dangereuses.",
        ),
    "featureCategoryTypeLight": MessageLookupByLibrary.simpleMessage(
      "Éclairage & électricité",
    ),
    "featureCategoryTypeLightDescription": MessageLookupByLibrary.simpleMessage(
      "Signalez des lampadaires cassés, des fils exposés ou des coupures de courant qui affectent les espaces publics et la sécurité.",
    ),
    "featureCategoryTypeRoad": MessageLookupByLibrary.simpleMessage(
      "Routes & transports",
    ),
    "featureCategoryTypeRoadDescription": MessageLookupByLibrary.simpleMessage(
      "Signalez des nids-de-poule, des routes endommagées, des trottoirs cassés, des feux de circulation défectueux ou une conduite imprudente dans votre région.",
    ),
    "featureCategoryTypeSecurity": MessageLookupByLibrary.simpleMessage(
      "Sécurité",
    ),
    "featureCategoryTypeSecurityDescription":
        MessageLookupByLibrary.simpleMessage(
          "Signalez des zones dangereuses, des infrastructures de sécurité endommagées, du harcèlement de rue ou des zones de regroupement.",
        ),
    "featureCategoryTypeUrban": MessageLookupByLibrary.simpleMessage(
      "Violations de zonage",
    ),
    "featureCategoryTypeUrbanDescription": MessageLookupByLibrary.simpleMessage(
      "Signalez des constructions non autorisées, des nuisances sonores, une mauvaise utilisation des terrains ou des violations de zonage dans votre zone.",
    ),
    "featureCategoryTypeWaste": MessageLookupByLibrary.simpleMessage("Déchets"),
    "featureCategoryTypeWasteDescription": MessageLookupByLibrary.simpleMessage(
      "Signalez des ordures non collectées, des drains obstrués, des dépôts illégaux ou d\'autres problèmes environnementaux.",
    ),
    "featureCompleteProfile": MessageLookupByLibrary.simpleMessage(
      "Compléter le profil",
    ),
    "featureDontHaveAccount": MessageLookupByLibrary.simpleMessage(
      "Vous n\'avez pas de compte ?",
    ),
    "featureForgotPassword": MessageLookupByLibrary.simpleMessage(
      "Mot de passe oublié ?",
    ),
    "featureForgotPasswordGoTo": MessageLookupByLibrary.simpleMessage(
      "Retour à",
    ),
    "featureForgotPasswordHint": MessageLookupByLibrary.simpleMessage(
      "Commencez la réinitialisation en remplissant votre adresse e-mail",
    ),
    "featureLoginWelcome": MessageLookupByLibrary.simpleMessage(
      "Content de vous revoir !",
    ),
    "featureLoginWelcomeHint": MessageLookupByLibrary.simpleMessage(
      "Connectez-vous à votre compte",
    ),
    "featureMyReports": MessageLookupByLibrary.simpleMessage(
      "Mes signalements",
    ),
    "featureNotificationEmpty": MessageLookupByLibrary.simpleMessage(
      "Vous n\'avez aucune notification",
    ),
    "featureNotificationInstruction": MessageLookupByLibrary.simpleMessage(
      "Marquer tout comme lu",
    ),
    "featureNotifications": MessageLookupByLibrary.simpleMessage(
      "Notifications",
    ),
    "featureOnboardingConnectVoice": MessageLookupByLibrary.simpleMessage(
      "Connectez-vous avec d\'autres voix",
    ),
    "featureOnboardingConnectVoiceDesc": MessageLookupByLibrary.simpleMessage(
      "Civic24 offre une plateforme pour interagir avec vos concitoyens en exprimant votre opinion sur des enjeux civiques",
    ),
    "featureOnboardingVoiceCount": MessageLookupByLibrary.simpleMessage(
      "Faites compter votre voix",
    ),
    "featureOnboardingVoiceCountDesc": MessageLookupByLibrary.simpleMessage(
      "Avec Civic24, vos avis aident à prioriser les problèmes civiques les plus importants pour les gens autour de vous",
    ),
    "featureOnboardingVoiceReport": MessageLookupByLibrary.simpleMessage(
      "Exprimez votre signalement",
    ),
    "featureOnboardingVoiceReportDesc": MessageLookupByLibrary.simpleMessage(
      "Civic24 vous permet de parler des problèmes urgents de votre communauté et de faire entendre votre voix",
    ),
    "featureProfileAccountEmail": MessageLookupByLibrary.simpleMessage(
      "L\'adresse e-mail de votre compte ne peut pas être modifiée.",
    ),
    "featureReports": MessageLookupByLibrary.simpleMessage("Rapports"),
    "featureSettingsAbout": MessageLookupByLibrary.simpleMessage("À propos"),
    "featureSettingsAppearance": MessageLookupByLibrary.simpleMessage(
      "Apparence",
    ),
    "featureSettingsAppearanceDark": MessageLookupByLibrary.simpleMessage(
      "Mode sombre",
    ),
    "featureSettingsAppearanceDarkDesc": MessageLookupByLibrary.simpleMessage(
      "Conserver l\'application en mode sombre",
    ),
    "featureSettingsAppearanceLight": MessageLookupByLibrary.simpleMessage(
      "Mode clair",
    ),
    "featureSettingsAppearanceLightDesc": MessageLookupByLibrary.simpleMessage(
      "Conserver l\'application en mode clair",
    ),
    "featureSettingsAppearanceSystem": MessageLookupByLibrary.simpleMessage(
      "Paramètres système",
    ),
    "featureSettingsAppearanceSystemDesc": MessageLookupByLibrary.simpleMessage(
      "S\'ajuste automatiquement aux paramètres d\'apparence de votre appareil",
    ),
    "featureSettingsEditProfile": MessageLookupByLibrary.simpleMessage(
      "Modifier le profil",
    ),
    "featureSettingsLanguage": MessageLookupByLibrary.simpleMessage("Langue"),
    "featureSettingsLanguageDefault": MessageLookupByLibrary.simpleMessage(
      "Langue par défaut",
    ),
    "featureSettingsLoading": MessageLookupByLibrary.simpleMessage(
      "Chargement",
    ),
    "featureSettingsLoggingOut": MessageLookupByLibrary.simpleMessage(
      "Déconnexion ?",
    ),
    "featureSettingsLoggingOutHint": MessageLookupByLibrary.simpleMessage(
      "Êtes-vous sûr de vouloir vous déconnecter de votre compte sur cet appareil ?",
    ),
    "featureSettingsProfile": MessageLookupByLibrary.simpleMessage("Profil"),
    "featureSettingsSecurityApple": MessageLookupByLibrary.simpleMessage(
      "Apple",
    ),
    "featureSettingsSecurityAppleLinkedDesc":
        MessageLookupByLibrary.simpleMessage(
          "Votre identifiant Apple est disponible pour la connexion",
        ),
    "featureSettingsSecurityAppleUnlinkDesc":
        MessageLookupByLibrary.simpleMessage(
          "Associez votre identifiant Apple pour une connexion sécurisée",
        ),
    "featureSettingsSecurityGoogle": MessageLookupByLibrary.simpleMessage(
      "Google",
    ),
    "featureSettingsSecurityGoogleLinkedDesc":
        MessageLookupByLibrary.simpleMessage(
          "Votre compte Google est disponible pour la connexion",
        ),
    "featureSettingsSecurityGoogleUnlinkDesc":
        MessageLookupByLibrary.simpleMessage(
          "Associez votre compte Google pour une connexion sécurisée",
        ),
    "featureSettingsSecurityResetPassword":
        MessageLookupByLibrary.simpleMessage("Changer le mot de passe"),
    "featureSettingsSecurityResetPasswordDesc":
        MessageLookupByLibrary.simpleMessage(
          "Mettez à jour votre mot de passe existant",
        ),
    "featureSettingsSecuritySetPassword": MessageLookupByLibrary.simpleMessage(
      "Définir un mot de passe",
    ),
    "featureSettingsSecuritySetPasswordDesc":
        MessageLookupByLibrary.simpleMessage(
          "Activer la connexion par e-mail et mot de passe",
        ),
    "featureSettingsSecuritySetPasswordLabel":
        MessageLookupByLibrary.simpleMessage(
          "Saisissez l\'e-mail associé à votre compte",
        ),
    "featureSettingsSignInSecurity": MessageLookupByLibrary.simpleMessage(
      "Connexion et sécurité",
    ),
    "featureSettingsTooltip": m0,
    "featureSettingsVersion": m1,
    "featureSignUpCreateAccount": MessageLookupByLibrary.simpleMessage(
      "Créer un compte",
    ),
    "featureSignUpCreateAccountHint": MessageLookupByLibrary.simpleMessage(
      "Inscrivez-vous pour commencer !",
    ),
    "featureSignUpSuccess": MessageLookupByLibrary.simpleMessage("Compte créé"),
    "featureSignUpSuccessHint": MessageLookupByLibrary.simpleMessage(
      "Préparez-vous à découvrir les fonctionnalités incroyables de Civic24.",
    ),
    "featureonboardingGetStarted": MessageLookupByLibrary.simpleMessage(
      "Commencer",
    ),
    "generalAddComment": MessageLookupByLibrary.simpleMessage(
      "Ajouter un commentaire",
    ),
    "generalAllIssues": MessageLookupByLibrary.simpleMessage(
      "Tous les problèmes",
    ),
    "generalCancel": MessageLookupByLibrary.simpleMessage("Annuler"),
    "generalCategory": MessageLookupByLibrary.simpleMessage("Catégorie"),
    "generalClickToUpload": MessageLookupByLibrary.simpleMessage(
      "Cliquez pour télécharger",
    ),
    "generalClose": MessageLookupByLibrary.simpleMessage("Fermer"),
    "generalComment": MessageLookupByLibrary.simpleMessage("Commentaires"),
    "generalConfirmPassword": MessageLookupByLibrary.simpleMessage(
      "Confirmer le mot de passe",
    ),
    "generalContinue": MessageLookupByLibrary.simpleMessage("Continuer"),
    "generalContinueWithApple": MessageLookupByLibrary.simpleMessage(
      "Continuer avec Apple",
    ),
    "generalContinueWithGoogle": MessageLookupByLibrary.simpleMessage(
      "Continuer avec Google",
    ),
    "generalCountry": MessageLookupByLibrary.simpleMessage("Pays"),
    "generalDeleteAccountButton": MessageLookupByLibrary.simpleMessage(
      "Supprimer le compte",
    ),
    "generalEdit": MessageLookupByLibrary.simpleMessage("Modifier"),
    "generalEmail": MessageLookupByLibrary.simpleMessage("Adresse e-mail"),
    "generalEmailHint": MessageLookupByLibrary.simpleMessage(
      "Entrez votre e-mail",
    ),
    "generalEnterEmail": MessageLookupByLibrary.simpleMessage(
      "Entrez votre adresse e-mail",
    ),
    "generalEnterFirstname": MessageLookupByLibrary.simpleMessage(
      "Entrez votre prénom",
    ),
    "generalEnterLastname": MessageLookupByLibrary.simpleMessage(
      "Entrez votre nom",
    ),
    "generalEnterPassword": MessageLookupByLibrary.simpleMessage(
      "Entrez votre mot de passe",
    ),
    "generalFirstname": MessageLookupByLibrary.simpleMessage("Prénom"),
    "generalForgotPassword": MessageLookupByLibrary.simpleMessage(
      "Mot de passe oublié ?",
    ),
    "generalLastname": MessageLookupByLibrary.simpleMessage("Nom de famille"),
    "generalLogin": MessageLookupByLibrary.simpleMessage("Connexion"),
    "generalLogout": MessageLookupByLibrary.simpleMessage("Se déconnecter"),
    "generalNoResults": MessageLookupByLibrary.simpleMessage("Aucun résultat"),
    "generalPassword": MessageLookupByLibrary.simpleMessage("Mot de passe"),
    "generalPrevious": MessageLookupByLibrary.simpleMessage("Précédent"),
    "generalProceed": MessageLookupByLibrary.simpleMessage("Procéder"),
    "generalRequired": MessageLookupByLibrary.simpleMessage("Requis"),
    "generalResetPassword": MessageLookupByLibrary.simpleMessage(
      "Réinitialiser le mot de passe",
    ),
    "generalResetPasswordButtonText": MessageLookupByLibrary.simpleMessage(
      "Ouvrir l\'application e-mail",
    ),
    "generalResetPasswordDesc": m2,
    "generalSave": MessageLookupByLibrary.simpleMessage("Enregistrer"),
    "generalSearch": MessageLookupByLibrary.simpleMessage("Rechercher ici"),
    "generalSelectCountry": MessageLookupByLibrary.simpleMessage(
      "Sélectionnez un pays",
    ),
    "generalSelectState": MessageLookupByLibrary.simpleMessage(
      "Sélectionnez une région",
    ),
    "generalSignUp": MessageLookupByLibrary.simpleMessage("S\'inscrire"),
    "generalState": MessageLookupByLibrary.simpleMessage("État/Région"),
    "generalSubmit": MessageLookupByLibrary.simpleMessage("Soumettre"),
    "generalSupportedFormats": MessageLookupByLibrary.simpleMessage(
      "Formats pris en charge : jpg, png, mp4 (max. 800x400px)",
    ),
    "generalTakePicture": MessageLookupByLibrary.simpleMessage(
      "Prendre une photo",
    ),
    "generalTrending": MessageLookupByLibrary.simpleMessage("Tendances"),
    "generalUploadFile": MessageLookupByLibrary.simpleMessage(
      "Télécharger le fichier",
    ),
    "generalUseAccurateLocation": MessageLookupByLibrary.simpleMessage(
      "Utiliser une position précise",
    ),
    "generalValidEmail": MessageLookupByLibrary.simpleMessage(
      "Une adresse e-mail valide est requise",
    ),
    "generalValidPassword": MessageLookupByLibrary.simpleMessage(
      "Le mot de passe doit comporter au moins 8 caractères",
    ),
    "pageHome": MessageLookupByLibrary.simpleMessage("Accueil"),
    "pageNotifications": MessageLookupByLibrary.simpleMessage("Notifications"),
    "pageReports": MessageLookupByLibrary.simpleMessage("Rapports"),
    "pageSettings": MessageLookupByLibrary.simpleMessage("Paramètres"),
    "title": MessageLookupByLibrary.simpleMessage("Civic24"),
  };
}
